...
public class AvlTree extends BinarySearchTreeRecursive {
  ...
  Node insertNode(int key, Node node) {
    ...
    public int encontrarProfundidade(int valor) {
        return encontrarProfundidade(valor, root, 0);
    }


    private int encontrarProfundidade(int key, Node node, int prof) {
        if (node == null) {
            return -1; // chave não encontrada
        }

        if (node.data == key) {
            return prof; // achou o nó → retorna o nível atual
        }

        if (key < node.data) {
            return encontrarProfundidade(key, node.left, prof + 1); // desce à esquerda
        } else {
            return encontrarProfundidade(key, node.right, prof + 1); // desce à direita
        }
    }
